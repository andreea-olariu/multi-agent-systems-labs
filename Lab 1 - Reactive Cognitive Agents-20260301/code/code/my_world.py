...
class MyPredatorWithCommunication(MyPredator):
    ...
    def response(self, perceptions: MyAgentPerception) -> SocialAction:
        ...
        # if agent sees prey, target the closest one; else, if it receives messages, target the closest reported prey; else, move randomly
        if perceptions.nearby_prey:
            target_prey = min(perceptions.nearby_prey, 
                            key=lambda p: agent_pos.get_distance_to(p[1]))
            prey_id, prey_pos = target_prey

        elif perceptions.messages:
            close_reports = []
            for msg in perceptions.messages:
                prey_id, prey_pos = msg.content
                distance = agent_pos.get_distance_to(prey_pos)
                
                if distance <= 4:
                    close_reports.append((prey_id, prey_pos))
            
            if close_reports:
                target_prey = min(close_reports,
                                key=lambda p: agent_pos.get_distance_to(p[1]))
                prey_id, prey_pos = target_prey
            else:
                target_prey = None
        ...
        chosen_action = probability_map.choice() if not probability_map.empty() else available_moves.choice()
        
        social_action = SocialAction(chosen_action)

        for (prey_id, prey_pos) in perceptions.nearby_prey:
            prey_distance = agent_pos.get_distance_to(prey_pos)

            if prey_distance <= 2:
                for (predator_id, predator_pos) in perceptions.nearby_predators:
                    predator_distance = agent_pos.get_distance_to(predator_pos)
                    
                    if predator_distance <= 2:
                        social_action.add_outgoing_message(
                            sender_id=self.id,
                            destination_id=predator_id,
                            content=(prey_id, prey_pos)
                        )
...
if __name__ == "__main__":
    ...
    # tester_w_communication = Tester(predator_agent_type=MyPredatorWithCommunication, rand_seed=42, delay=0.1)
    # step_count_comm, prey_kill_times_comm = tester_w_communication.make_steps()
    # print("Step count with communication: ", step_count_comm)
    # print("Prey kill times with communication: ", prey_kill_times_comm)

    NUM_TESTS = 20

    def run_tests(num_tests: int = NUM_TESTS, predator_agent_type = MyPredator, img_name = "prey_kill_times_no_comm.png"):
        step_count_list = []
        prey_kill_times_list = []

        for i in range(num_tests):
            tester = Tester(predator_agent_type=predator_agent_type, rand_seed=42+i, delay=0.1)
            step_count, prey_kill_times = tester.make_steps()

            step_count_list.append(step_count)
            prey_kill_times_list.append(prey_kill_times)
    
        print("Step count analysis without communication")
        text = pd.Series(step_count_list).describe()

        with open("step_count_analysis.txt", "a") as f:
            f.write(str(text))
    ...
    run_tests(num_tests=NUM_TESTS, predator_agent_type=MyPredator, img_name="prey_kill_times_no_comm.png")
    run_tests(num_tests=NUM_TESTS, predator_agent_type=MyPredatorWithCommunication, img_name="prey_kill_times_with_comm.png")
