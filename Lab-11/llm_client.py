...
DEFAULT_MODEL       = os.getenv("GEMINI_MODEL", os.getenv("OPENAI_MODEL", "gpt-4o-mini"))
...
def _get_client() -> OpenAI:
    ...
    if _client is None:
        ...
        if not api_key:
            raise RuntimeError(
                "GEMINI_API_KEY or OPENAI_API_KEY is not set. Copy .env.example to .env and fill it in."
            )
        
        # If the key starts with "AIza", it's a Google Gemini key.
        # We need to point the OpenAI client to Google's compatible endpoint.
        if api_key.startswith("AIza"):
            ...
            # Otherwise, use the standard OpenAI endpoint
...
def call_llm(
    ...
    # --- AGGRESSIVE RATE LIMIT RETRY LOOP ---
    max_retries = 10
    resp = None
    ...
    for attempt in range(max_retries):
        try:
            ...
            # We catch a generic Exception and look for the 429 code 
            # to guarantee it doesn't slip through the cracks.
            error_string = str(e)
            if "429" in error_string or "Quota" in error_string or "RateLimit" in error_string:
                wait_time = 65
                logger.warning(f"\n[Rate Limit] Agents talking too fast! Sleeping for {wait_time}s... (Attempt {attempt + 1}/{max_retries})")
                time.sleep(wait_time)
                if attempt == max_retries - 1:
                    raise RuntimeError("Exhausted all retries waiting for rate limits.") from e
            else:
                # If it's some other error (like network down), crash normally
                raise e
    # ----------------------------------------
    ...
    # --- ADD THIS BLOCK TO CLEAN MARKDOWN ---
    ...
    print(f"\n[DEBUG] Raw LLM Output: {repr(content)}\n")
