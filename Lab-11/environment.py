...
if __name__ == "__main__":
    ...
    try:
        from llm_client import session_summary
        summary = session_summary()
        logger.info("[LLM Session Summary] calls=%d tokens=%d cost=$%.6f",
                    summary["calls"], summary["tokens"], summary["cost_usd"])
        print("\n[LLM Session Summary]", summary)
    except Exception as e:
        logger.warning("Could not produce LLM session summary: %s", e)
